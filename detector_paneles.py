...
class DetectorPaneles:
    ...
    def buscar_rectangulos_grandes(self, imagen):
        """PLAN B: Detecta paneles gigantes cercanos (Canny + Contornos)."""
        detecciones_extra = []
    ...
    def buscar_paneles_niebla(self, imagen):
        """PLAN C (Quirúrgico): Busca solo azul desaturado en el cielo."""
        detecciones_niebla = []
        alto_img, ancho_img = imagen.shape[:2]

        # 1. Recortamos: Solo miramos la mitad superior de la imagen (evita coches y asfalto)
        limite_y = int(alto_img * 0.5)
        roi_superior = imagen[0:limite_y, 0:ancho_img]

        # 2. Rango HSV de la Niebla: 
        # Exigimos tono azul (95-135), pero Saturación MUY BAJA (15-110)
        # Al poner máximo 110 de saturación, ignoramos el cielo azul despejado automáticamente.
        ...
        # 3. Limpieza morfológica para unir el panel y borrar ruido de las nubes
        ...
        for cnt in contornos:
            area = cv2.contourArea(cnt)
            # 4. Solo paneles con un tamaño decente (la niebla no deja ver los pequeñitos)
            if 1500 < area < 40000:
                x, y, w, h = cv2.boundingRect(cnt)
                relacion_aspecto = w / float(h)

                # 5. Geometría estricta de panel
                if 0.8 < relacion_aspecto < 3.5:
                    # Le damos un score moderado fijo (0.45). 
                    # Así, si el Plan A también lo vio, ganará el Plan A en la criba final.
    ...
    def detectar(self, imagen):
        ...
        # Sumamos todos los cazadores
        detecciones_totales = detecciones_iniciales + detecciones_extra + detecciones_niebla

        # --- LIMPIEZA FINAL NMS ---
        # El NMS se encargará de fusionar si el Plan A y el Plan C cazaron lo mismo
